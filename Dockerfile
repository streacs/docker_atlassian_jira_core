...
RUN set -x \
  ...
  && wget --no-check-certificate -nv -O /tmp/atlassian-jira-core-${APPLICATION_RELEASE}.tar.gz https://www.atlassian.com/software/jira/downloads/binary/atlassian-jira-core-${APPLICATION_RELEASE}.tar.gz \
  && tar xfz /tmp/atlassian-jira-core-${APPLICATION_RELEASE}.tar.gz --strip-components=1 -C ${APPLICATION_INST} \
  && chown -R ${SYSTEM_USER}:${SYSTEM_GROUP} ${APPLICATION_INST} \
  && chown -R ${SYSTEM_USER}:${SYSTEM_GROUP} ${APPLICATION_HOME} \
  && rm /tmp/atlassian-jira-core-${APPLICATION_RELEASE}.tar.gz
